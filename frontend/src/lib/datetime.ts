/**
 * Centralized Date/Time Utility Module (Frontend)
 * 
 * CRITICAL: All date/time operations in the frontend MUST use this module
 * to ensure consistency with backend and correctness.
 * 
 ...
 * - Integration with date-fns for safe operations
 */
 ...
 * This is the SINGLE SOURCE OF TRUTH for "now" across the entire frontend
 * 
 * @returns Date object representing current time in IST
 * 
 * @example
 * const now = getNowIST();
 * console.log(now); // Current time in IST
 */
 ...
 * 
 * @example
 * const hour = getHourIST(new Date()); // Returns current hour in IST
 */
 ...
 * 
 * @example
 * formatDateIST(new Date(), 'MMM d, yyyy') // Returns "Dec 22, 2025"
 */
export function formatDateIST(date: Date, formatString: string): string {
  return dateFnsFormat(date, formatString);
}

/**
 * Format date for display with locale options in IST timezone
 * 
 * @param date - Date to format
 ...
 * 
 * @example
 * formatDateLocaleIST(new Date()) // Returns "Sun, Dec 22, 2025, 06:16 AM"
 */
export function formatDateLocaleIST(
  date: Date, 
 ...
 * // Returns:
 * // [
 * //   { value: "2025-12-26", label: "Current Week (Dec 19 - now)", weekStart: Dec 19 5pm, weekEnd: now },
 * //   { value: "2025-12-19", label: "Dec 12 - Dec 19, 2025", weekStart: Dec 12 5pm, weekEnd: Dec 19 5pm },
 * //   { value: "2025-12-12", label: "Dec 5 - Dec 12, 2025", weekStart: Dec 5 5pm, weekEnd: Dec 12 5pm }
 * // ]
 */
export function getWeekOptionsIST(numberOfWeeks: number = 2): Array<{
  ...
  for (let i = 0; i < numberOfWeeks; i++) {
    ...
    options.push({
      value: dateFnsFormat(weekEnd, 'yyyy-MM-dd'),
      label: `${dateFnsFormat(weekStart, 'MMM d')} - ${dateFnsFormat(weekEnd, 'MMM d, yyyy')}`,
      weekStart,
      weekEnd,
    });
  }
