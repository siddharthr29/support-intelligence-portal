    ...
    try {
      ...
      const partners = await prisma.$queryRaw<Array<any>>`
        SELECT 
          ...
          COUNT(*) FILTER (WHERE t.status IN (4, 5)) as resolved_count,
      ...
      const partnersWithNumbers = partners.map(p => ({
        ...
        resolved_count: Number(p.resolved_count),
        avg_resolution_hours: p.avg_resolution_hours ? Math.round(Number(p.avg_resolution_hours) * 10) / 10 : null,
